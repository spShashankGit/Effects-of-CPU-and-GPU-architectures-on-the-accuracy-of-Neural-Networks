...
def main(seed_value=0):
    ...
    vggCPUAda = VGG_11()  # Initialize model on CPU
    vggCPUAda = vggCPUAda.double()
    #vggGPUAda = vggGPUAda.to(device="cuda")

    # 5 - Initialize the optimizer
    optimizer = optim.SGD(vggCPUAda.parameters(), lr=0.1, momentum=0.9, nesterov=False)
